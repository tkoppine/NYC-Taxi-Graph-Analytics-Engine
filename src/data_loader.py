...
class DataLoader:
    ...
    def load_transform_file(self, file_path):
        ...
        # To create locations
        def locations_nodes(session, locations):
            for location in locations:
                session.run("MERGE (l:Location {name: $name})", name=int(location))
                
        # To create trips
        def trips_nodes(session, trips):
        ...
        with self.driver.session() as session:
            lctns = set(trips['PULocationID']).union(set(trips['DOLocationID']))
            locations_nodes(session, lctns)
            trips_nodes(session, trips)
